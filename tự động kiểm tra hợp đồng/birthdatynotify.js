/**
 * BirthdayNotify.js
 * Trigger chạy mỗi ngày, nếu có sinh nhật hôm nay thì gửi email thông báo.
 *
 * Nguồn dữ liệu: sheet "sinh nhật" trong file Thưởng (Spreadsheet hiện tại),
 * cột C = ngày, D = tháng.
 */
...
function dailyBirthdayNotify() {
  ...
  function sendBirthdayNotifications_(options) {
    ...
    const recipientsKey = BONUS_CONFIG.BIRTHDAY.PROP_RECIPIENTS;
    const lastSentKey = BONUS_CONFIG.BIRTHDAY.PROP_LAST_SENT_DATE;
    const recipientsRaw = String(props.getProperty(recipientsKey) || "").trim();
    const recipients = buildRecipientsList_(recipientsRaw, [
    //   "800@add-group.net",
    //   "lookingboy2003@gmail.com"
    ...
    const sheet = getBirthdaySheet_(ss);
    if (!sheet) {
      return { sent: false, message: "❌ Không tìm thấy sheet sinh nhật." };
    }
  
    const lastRow = sheet.getLastRow();
    if (lastRow < 2) {
      return { sent: false, message: "ℹ️ Sheet sinh nhật chưa có dữ liệu." };
    }
  
    const cols = BONUS_CONFIG.BIRTHDAY.COLUMNS;
    const width = cols.ID; // đọc tới cột I
    ...
    values.forEach((row) => {
      ...
      const d = parseNumber_(row[cols.DAY - 1]);
      const m = parseNumber_(row[cols.MONTH - 1]);
      if (!d || !m) return;
  
      if (d === day && m === month) {
        birthdayToday.push({
          name: name,
          company: String(row[cols.COMPANY - 1] || "").trim(),
          id: String(row[cols.ID - 1] || "").trim()
        });
      }
    });
  ...
  function getBirthdaySheet_(ss) {
    const candidates = BONUS_CONFIG.BIRTHDAY.SHEET_NAME_CANDIDATES || [];
  ...
  function getBirthdayTriggerStatus() {
    ...
    const props = PropertiesService.getScriptProperties();
    const savedHour = props.getProperty(BONUS_CONFIG.BIRTHDAY.PROP_HOUR);
    const hour = savedHour ? parseInt(savedHour, 10) : 8;
    const recipients = props.getProperty(BONUS_CONFIG.BIRTHDAY.PROP_RECIPIENTS) || "";
  ...
  function installBirthdayTriggerWithHour(hour, recipients) {
    ...
    const props = PropertiesService.getScriptProperties();
    props.setProperty(BONUS_CONFIG.BIRTHDAY.PROP_HOUR, String(Number(hour) || 8));
    props.setProperty(BONUS_CONFIG.BIRTHDAY.PROP_RECIPIENTS, String(recipients || "").trim());
  ...
  function testBirthdayEmail() {
    return runBirthdayNotifyFromUI();
  }
